...
@export var forca_knockback = 15.0 # Aumentei um pouco para sentir o impacto
...
func decidir_direcao():
	...
	if sorteio <= chance_acerto:
		...
		var desvio = deg_to_rad(randf_range(30, 60))
...
func _on_area_de_knockpack_e_dano_laranja_body_entered(body: Node3D) -> void:
	if body.is_in_group("player"):
		...
		# 2. Calcula Knockback (Empurra para longe do centro do bicho)
		var direcao_kb = (body.global_position - global_position).normalized()
		
		# Ajuste: Faz o player "voar" levemente (eixo Y) e empurra no X e Z
		direcao_kb.x = sorteio
		
		if "velocity" in body:
			body.velocity = direcao_kb * forca_knockback
			# Se o player usar move_and_slide, o impacto será imediato
